...
**Refund policy** (`app/models/refund_policy.rb`, Ticket D) is a plain value object with no table — `events.refund_policy_tiers` and `registrations.refund_policy_tiers` are jsonb arrays of `{hours_before, refund_percent}` that get handed to it. Three states are deliberately distinguishable and must not be collapsed: `nil` (host set no policy — refunds stay a manual decision, and this is what every pre-existing event has), `[]` (an explicit "no refunds"), and populated tiers. Correspondingly `Registration#refund_entitlement_cents` returns `nil` rather than `0` when there's no policy, because "a human decides" and "the policy says nothing comes back" are different answers. The policy is snapshotted onto the registration by a `before_validation on: :create` callback in the model — not at the three call sites that create registrations — so a host tightening their policy can't retroactively reduce what someone already agreed to at checkout. `RefundPolicy#refund_percent_for` is pure and order-independent (it takes the most generous qualifying tier), so a legacy or hand-edited row still gets a defensible answer even though ordering is validated on the way in.

**Participant self-cancellation** (`app/services/registrations/cancel.rb`, Ticket E) is the automated counterpart to `Refunds::IssueRefund` — it decides the amount from the registration's snapshotted policy and nobody approves it. It builds on `IssueRefund` rather than replacing it, with one correction: `IssueRefund` only cancels a registration when a refund exhausts its payment, but a participant taking a 50% refund has still given up their spot, so `Cancel` cancels separately and guards against promoting from the waitlist twice for one freed slot. `POST /api/v1/registrations/:id/cancel` is owner-or-admin only; an organizer removing a participant is still `#destroy`, a different action. A paid registration on an event with no policy returns **409 `requires_organizer`** rather than cancelling — silently cancelling would cost the participant money the host might have returned.

**`HostLedgerEntry`** (Ticket F) is the running account between Rally and a host organization, needed because splitting at capture pays the host before the event, so a later refund is a clawback rather than a reversal. **Append-only**: `before_update`/`before_destroy` raise, and corrections are compensating entries. `amount_cents` is signed (positive = owed to the host) so the balance is a plain `SUM` that can't disagree with itself — `Organization#ledger_balance_cents` derives it rather than caching it, since a cached total is precisely the thing that drifts from the entries. **Nothing writes to this table yet**; it landed early because it has no ABA dependency.
