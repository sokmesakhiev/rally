...
### Frontend: the sitemap is built, not served

`sitemap.xml` and the `Sitemap:` line in `robots.txt` are generated by
`scripts/build-sitemap.mjs` after `vite build`, writing into `dist/client/`.
The logic lives in `src/lib/sitemap.ts` and is unit-tested; the script only
reads env, fetches, and writes.

- **It used to be a route, and that could never have worked.** `sitemap[.]xml.ts`
  exported a `server:` handler, and this app has no server — no nitro plugin,
  and the deploy syncs `dist/client/` only. Worse than a 404: CloudFront maps
  404 → 200 `/index.html`, so `GET /sitemap.xml` returned the SPA shell with
  `Content-Type: text/html` and a 200, which Search Console reads as an
  *unreadable* sitemap rather than a missing one. It worked perfectly under
  `npm run dev`, which is why it survived. **Anything that must exist at a URL
  in production has to be a file in `dist/client/`, not a route handler.**
- **Nothing degrades.** A missing `VITE_BASE_URL`, an unset `VITE_API_URL`, an
  unreachable API or any non-2xx all exit 1 and fail the build. The bug this
  replaced swallowed a 422 into an empty list and shipped a two-entry sitemap
  that looked exactly like a working one.
- **`VITE_BASE_URL` is now required to build** (`.env.example`,
  `vars.VITE_BASE_URL` in `deploy.yml`, `tf_output frontend_url` in
  `scripts/deploy.sh`). It was referenced in three files and defined in none,
  so it was always `""` — every `<loc>` came out relative, and so do the
  canonical/`og:url` tags on the event and organizer pages, **which are still
  unfixed**.
- **The script reads `.env` files itself** (`resolveEnv`), because it's a
  separate `node` process and inherits only `process.env` — Vite's env loading
  applies to the bundle, not to it. Without this the workflow `.env.example`
  documents ("copy to .env.local, then `npm run build`") failed with
  "VITE_BASE_URL is not set", i.e. the fail-loudly path firing for the wrong
  reason. Deliberately **not** Vite's `loadEnv`: importing `vite` drags in
  rolldown's platform-specific native binding to read a few `key=value` lines.
  Real environment variables beat the files, so CI and `scripts/deploy.sh`
  still win.
- **`engines.node` is `>=22.18`, enforced by `.npmrc`'s `engine-strict`.** The
  script imports a `.ts` module, which Node runs directly only from 22.18 (type
  stripping on by default). Below that it dies with
  `ERR_UNKNOWN_FILE_EXTENSION`, which names nothing useful. CI and deploy are
  both on Node 24, so this only ever bites a local checkout — which is where a
  legible message is worth most.
- **`SITEMAP_MAX_PER_PAGE` is coupled to `EventIndexRequestSchema::MAX_PER_PAGE`**
  (50) across two repositories of truth, so a test pins it. The old route asked
  for 1000 and got exactly the 422 that schema's comment promises.
- **The event list must come from `/api/v1/events`** — that's
  `Event.publicly_visible`, which is what keeps unlisted events out. A sitemap
  is the most literal "listing to strangers" there is; a test pins the endpoint
  so a future change can't route around the scope to dodge the page cap.
- **Consequence to know:** the sitemap is only as fresh as the last *frontend*
  deploy. A new event doesn't appear until someone ships frontend code. If that
  becomes a problem, the fix is a scheduled workflow that regenerates and
  uploads `sitemap.xml` alone — not a route.
